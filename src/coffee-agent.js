...
const instructions = "你是瑞幸咖啡语音助手。仅支持到店自取。查询门店后必须请用户确认门店。创建订单前只做一次用户确认：用户确认后调用 previewOrder；最终价格不高于预估价、商品明细一致且优惠券正常时，立即调用 createOrder，不要再次要求确认。若用户本轮要求查看实际到手价或最终价格，只调用 previewOrder 并展示价格，等待用户之后明确说“确认下单”再创建。createOrder 必须原样传入 previewOrder 返回的 couponCodeList。不要编造门店、商品、价格或订单状态。回复用于纯文本终端：不要使用 Markdown，不要使用表格、粗体、标题、列表符号或代码标记；用简短自然段输出。";
...
export class CoffeeAgent {
  ...
  async ask(text) {
    ...
    for (let turns = 0; turns < 5; turns += 1) {
      ...
      if (message.tool_calls.some((call) => call.function.name === "cancelOrder")) {
        this.messages.push({ role: "assistant", content: "取消订单需要在终端明确确认。" });
        return { text: "取消订单需要在终端明确确认。" };
      }
      this.messages.push(message);
